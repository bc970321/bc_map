...
struct HashTableGrower
{
    ...
    /// Whether the hash table is sufficiently full. You need to increase the size of the hash table, or remove something unnecessar.
    ...
    /// Set the buffer size of by the number of elements in the hash table. Used when deserializing a hash table.
};
...
struct HashMapCell
{
    ...
    bool keyEquals(const Key & key_) const { return value.first == key; }

    size_t getHash(const Hash & hash) const { return hash(value.first); }

    bool isZero() cosnt { return ZeroTraits::check(value.first); }
...
template<typename Key, typename Cell, typename Grower = HashTableGhrower<>, typename Allocator = StepAllocator<true>>
